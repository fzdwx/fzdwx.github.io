<!-- copy from https://github.com/yzh990918/evan-yang -->
<script lang="ts" setup>
const config = useAppConfig()
</script>

<template>
    <header>
        <div class="flex justify-between items-center max-w-8xl max-h-22 mx-auto sm:px-8 px-4">
            <div>
                logo
            </div>
            <div class="flex items-center">
                <!-- Navigation -->
                <nav aria-label="Main Menu" class="opacity-75">
                    <ul class="flex lg:(space-x-7 mr-15) sm:space-x-2 space-x-1 md:mr-7 items-center">
                        <li v-for="(link, key) in config.links" :key="key">
                            <NuxtLink v-slot="{ isActive }" :to="link.url" class="!inline-flex items-center group font-semibold" :title="`Visit ${link.title} Page`">
                                <div :class="isActive ? ['sm:bg-just-light/20', 'text-just-dark'] : []" class="md:(px-3 py-2) px-1 py-1 rounded">
                                    <svg
                                            v-if="link.icon"
                                            v-bind="link.icon"
                                            class="hidden sm:inline-block text mr-2 mb-1 transition opacity-75 group-hover:opacity-100"
                                            :class="isActive ? 'opacity-100' : 'opacity-75'"
                                    />
                                    {{ link.title }}
                                </div>
                                <Link :key="key" rel="prefetch" :href="link.path" />
                            </NuxtLink>
                        </li>
                    </ul>
                </nav>
            </div>
        </div>
    </header>
</template>

<style scoped>
header {
    border-style: none;
    position: absolute;
    top: 0;
    width: 100%;
    z-index: 50;
}
</style>
